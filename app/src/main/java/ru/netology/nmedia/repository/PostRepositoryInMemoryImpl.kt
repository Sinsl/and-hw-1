...
class PostRepositoryInMemoryImpl: PostRepository {
    ...
    override fun likeById(id: Long) {
        posts = posts.map {
            if (it.id != id) it else it.copy(likedByMe = !it.likedByMe, likeCount = if (it.likedByMe) it.likeCount - 1 else it.likeCount + 1)
        }
        data.value = posts
    }
    ...
    override fun viewById(id: Long) {
        posts = posts.map {
            if (it.id != id) it else it.copy(viewCount = it.viewCount + 1)
        }
        data.value = posts
    }
